...
def select_file():
    ...
    video_label.config(text=f"Выбранный файл: {file_name}", fg="green")
    global cap
    ...
    play_button = tk.Button(text="Воспроизвести", command=lambda: video_play(video_path))
...
def select_segment_time_in_seconds():
    def get_number(time):
        ...
        segment_time_label.config(text=f"Время одного сегмента --> {selected_segment_time} секунд", fg="green")
...
def select_project_name():
    def get_name(name):
        global project_name
        project_name = name
        project_name_label.config(text=f"Название проекта --> {project_name}", fg="green")
        saved_label.config(text="Сохранено!", fg="green")
        print(project_name)
...
def select_savepath():
    ...
    if folder_path:
        ...
        savepath_label.config(text=f"Путь для сохранения файлов -->{output_folder}", fg="green")
        print(output_folder)
...
left_canvas = tk.Canvas(window, width=300, height=600)
left_canvas.grid(row=0, column=0, rowspan=10, padx=25, pady=5, sticky="nsew")

project_name_select_button_label = tk.Label(left_canvas, text="Название проекта:")
project_name_select_button_label.grid(row=0, column=0, padx=0, pady=10, sticky="w")
project_name_select_button = tk.Button(left_canvas, text="Выбрать", command=select_project_name)
project_name_select_button.grid(row=0, column=1, padx=5, pady=10)

project_name_label = tk.Label(left_canvas, text="Название проекта -->", fg="red")
project_name_label.grid(row=1, column=0, padx=0, pady=10, sticky="w")

savepath_select_button_label = tk.Label(left_canvas, text="Куда сохранять:")
savepath_select_button_label.grid(row=2, column=0, padx=0, pady=10, sticky="w")
savepath_select_button = tk.Button(left_canvas, text="Выбрать", command=select_savepath)
savepath_select_button.grid(row=2, column=1, padx=5, pady=10)

savepath_label = tk.Label(left_canvas, text=f"Путь для сохранения файлов -->{output_folder}", fg="red")
savepath_label.grid(row=3, column=0, padx=0, pady=10, sticky="w")

file_label = tk.Label(left_canvas, text="Выберете .mp4 файл:")
file_label.grid(row=4, column=0, padx=0, pady=10, sticky="w")

select_button = tk.Button(left_canvas, text="Выбрать", command=select_file)
select_button.grid(row=4, column=1, padx=5, pady=10)

video_label = tk.Label(left_canvas, text="Выбранный файл:", fg="red")
...
canvas_video.create_text(150, 100, text="Выберете видео", font=("Helvetica", 18, "bold"), fill="#bd008b")

create_timecodes_button = tk.Button(left_canvas, text="Создать таймкоды из ссылки на ролик", command=create_timecodes)
create_timecodes_button.grid(row=7, column=0, columnspan=2, padx=0, pady=10, sticky="w")

view_timecodes_button = tk.Button(left_canvas, text="Посмотреть или изменить таймкоды", command=open_timecodes_table)
view_timecodes_button.grid(row=8, column=0, columnspan=2, padx=0, pady=10, sticky="w")

# Создание общего холста для кнопок и меток
right_canvas = tk.Canvas(window, width=800, height=300)
...
equal_time_button = tk.Button(buttons_frame, text="Нарезать равномерно по времени сегмента",
...
timecodes_button = tk.Button(buttons_frame, text="Нарезать просто по таймкодам",
...
segments_button = tk.Button(buttons_frame, text="Нарезать по таймкодам + сегментам",
...
# Метки
segment_time_label = tk.Label(labels_frame, text="Время одного сегмента -->   _______секунд", fg="red")
segment_time_label.grid(row=0, column=0, sticky="w")

# Кнопка "Указать"
segment_select_button = tk.Button(labels_frame, text="Указать", command=select_segment_time_in_seconds)
...
stop_button = tk.Button(bottom_buttons_frame, text="Остановить нарезку", command=lambda: uptade_stop_flag(), bg="#ffa483", fg="black")
stop_button.grid(row=0, column=0, padx=5)

continue_button = tk.Button(bottom_buttons_frame, text="Продолжить нарезку", command=lambda: uptade_stop_flag(), bg="#cbffca", fg="black")
